...
For example, I configured opening the terminal i3-style with `Hyper` + `Enter`, and once it launches, it's automatically moved to workspace 10. If the terminal is already open and I'm on a different workspace, pressing `Hyper` + `Enter` doesn't open a new window — AeroSpace switches me to the workspace where the terminal is already running (workspace 10).
